...
class FDIDMHardwareTestTab(QWidget):
    ...
    def _on_connect_clicked(self):
        try:
            ...
            self._log("FDIDM 严格论文链路后端已配置 v17.2（α/β 同步修复 + USRP 句柄清理 + 调试日志流）。")
            self._log(self._backend_summary())
            self.btn_connect.setEnabled(False)
            self.btn_start_test.setEnabled(True)
            self._set_hw_controls_enabled(False)
            # v17.2: do NOT draw a preview waveform here. The plots stay empty
            # until the user actually clicks "Start test" and live samples
            # start arriving from the GR pipeline.
            self._clear_plots()
            self.decode_status_label.setText("解调状态：已配置，未开始（点击 开始测试 启动 USRP 流）")
            # Drain any boot-time debug messages from the backend so the user
            # immediately sees the waveform fingerprint and config dump.
    ...
    def _refresh_plots(self):
        ...
        try:
            ...
            # RX spectrum: only plot if we actually have live samples; otherwise
            # leave the curve empty so the user can see at a glance whether the
            # USRP is producing data at all.
            rx_signal = np.asarray(self.backend.get_rx_spectrum_source(4096), dtype=np.complex64)
            if rx_signal.size == 0:
                self.rx_curve.setData([], [])
                self.rx_spectrum_plot.setTitle(
                    "USRP 接收基带频谱（无数据 — 第一块尚未到达 USRP 源）"
                )
            else:
                rx_freq, rx_psd = self._compute_spectrum(rx_signal, samp_rate, 1024)
                self.rx_curve.setData(rx_freq, rx_psd)
                self.rx_spectrum_plot.setTitle(
                    f"USRP 接收基带频谱（{rx_signal.size} 样点）"
                )
    ...
    def _refresh_tx_plot_only(self):
        """Repaint the TX plot once when the dropdown selection changes.

        v17.2: the backend now returns an empty array if there are no live
        samples in the TX buffer, so a 'preview' is impossible. The plot
        will be cleared until the user starts the test and USRP emits
        chunks back to the GR vector_sink.
        """
        try:
            if self.backend is None:
                return
    ...
    def _update_tx_plot(self, samp_rate: float):
        ...
        if "X_TF" in mode:
            # X_TF preview is a structural look at the IFDIT output for the
            # current parameters. It does not depend on live samples and is
            # safe to display before start().
            pts = self.backend.get_fdidm_preview_constellation("tf", 4096)
            y = np.abs(pts).astype(np.float64)
            x = np.arange(y.size, dtype=np.float64)
            self.tx_curve.setData(x, y)
            self.tx_plot.setTitle("X_TF 栅格幅度（论文 IFDIT 输出，结构预览）")
            ...
            return
        if "X 跨域" in mode:
            # Cross-domain constellation preview - structural, not live.
            pts = self.backend.get_fdidm_preview_constellation("cross", 4096)
            self.tx_curve.setData(np.real(pts), np.imag(pts))
            self.tx_plot.setTitle("X 跨域发送星座（QAM 映射后，结构预览）")
            self.tx_plot.setXRange(-2, 2, padding=0)
            self.tx_plot.setYRange(-2, 2, padding=0)
            return

        # Time-domain or spectrum: requires LIVE samples from GR vector_sink.
        sig = self.backend.get_tx_spectrum_source(4096)
        sig = np.asarray(sig, dtype=np.complex64)
        if sig.size == 0:
            # No live samples yet (configured but not started, or first
            # chunk hasn't arrived). Make this fact visible.
            self.tx_curve.setData([], [])
            self.tx_plot.setTitle(
                "TX 实时（无数据 — 未开始 USRP 流，或第一块尚未到达）"
            )
            return
        if "时域" in mode:
            y = np.abs(sig)
            x = np.arange(y.size, dtype=np.float64)
            self.tx_curve.setData(x, y)
            self.tx_plot.setTitle(f"TX 多帧时域幅度（实时，{sig.size} 样点）")
            self.tx_plot.setXRange(0, max(10, y.size), padding=0)
            ...
            self.tx_plot.setTitle(f"TX 基带频谱（USRP 实时发射流，{sig.size} 样点）")
