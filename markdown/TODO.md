...
- [ ] TODO: 软件仿真侧实现自适应：为 `simple_fdidm_rx.py`、`simple_otfs_rx.py`、`simple_ofdm_rx.py`、`simple_afdm_rx.py` 加入基于 CSI 的自适应模块
- [ ] TODO: 把 FDIDM 硬件现有的 α/β 预测优化抽象为通用接口，供其他波形复用
- [ ] TODO: 硬件侧为 OFDM / OTFS / AFDM 补齐等效的自适应能力
- [ ] TODO: 对比预测式自适应（基于信道模型）与反馈式自适应（基于实测 BER/EVM）的效果
- [ ] TODO: `compare_scan_backend.py` 增加“各波形自适应后”的公平对比模式
